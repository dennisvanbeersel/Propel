...
";
        ...
        if ($table && isset($changedProperties['autoIncrement'])) {
            // Auto-increment in PostgreSQL is emitted as a SQL:2003 IDENTITY column (the legacy
            // SERIAL/nextval() form was removed in 4.0), so add/drop the IDENTITY specification
            // rather than wiring up a sequence + nextval() default — the latter is rejected on
            // an identity column ("column ... is an identity column").
            if ($toColumn->isAutoIncrement() && $table->getIdMethodParameters() == null) {
                $ret .= sprintf($pattern, $this->quoteIdentifier($table->getName()), $colName . ' ADD GENERATED BY DEFAULT AS IDENTITY');
            } elseif (!$toColumn->isAutoIncrement() && $fromColumn->isAutoIncrement()) {
                $ret .= sprintf($pattern, $this->quoteIdentifier($table->getName()), $colName . ' DROP IDENTITY IF EXISTS');
            }
        }
