...
abstract class ValidateReaderBook implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Propel\\Tests\\Bookstore\\Behavior\\Map\\ValidateReaderBookTableMap';
    ...
    {
        if (isset($alreadyDumpedObjects['ValidateReaderBook'][$this->hashCode()])) {
            return ['*RECURSION*'];
        }
        $alreadyDumpedObjects['ValidateReaderBook'][$this->hashCode()] = true;
    ...
    {
        $criteria = ChildValidateReaderBookQuery::create();
        $criteria->add(ValidateReaderBookTableMap::COL_READER_ID, $this->reader_id);
        $criteria->add(ValidateReaderBookTableMap::COL_BOOK_ID, $this->book_id);
    ...
    public function validate(?ValidatorInterface $validator = null)
    {
        ...
        if (!$this->alreadyInValidation) {
            ...
            if (is_object($this->aValidateReader) and method_exists($this->aValidateReader, 'validate')) {
                if (!$this->aValidateReader->validate($validator)) {
                    $failureMap->addAll($this->aValidateReader->getValidationFailures());
                }
            }
            // If validate() method exists, the validate-behavior is configured for related object
            if (is_object($this->aValidateBook) and method_exists($this->aValidateBook, 'validate')) {
