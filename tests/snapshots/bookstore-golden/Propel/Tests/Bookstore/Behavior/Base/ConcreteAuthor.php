...
abstract class ConcreteAuthor implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Propel\\Tests\\Bookstore\\Behavior\\Map\\ConcreteAuthorTableMap';
    ...
    {
        $pos = (int)ConcreteAuthorTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['ConcreteAuthor'][$this->hashCode()])) {
            return ['*RECURSION*'];
        }
        $alreadyDumpedObjects['ConcreteAuthor'][$this->hashCode()] = true;
    ...
    {
        $criteria = ChildConcreteAuthorQuery::create();
        $criteria->add(ConcreteAuthorTableMap::COL_ID, $this->id);
        ...
        if (null === $this->collArticles || null !== $criteria || $partial) {
            if ($this->isNew()) {
                ...
                if (null === $this->collArticles) {
                    ...
                    $collArticles->setModel('\Propel\Tests\Bookstore\Behavior\ConcreteArticle');
     ...
     */
    protected function doAddArticle(ChildConcreteArticle $article): void
    {
