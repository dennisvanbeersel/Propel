...
class TimestampableBehaviorTest extends BookstoreTestBase
{
    ...
    {
        // Allow a couple of seconds of slack: with native ON UPDATE CURRENT_TIMESTAMP
        // (default on MySQL/MariaDB) the value is generated by the database server, so it
        // can lag or lead the PHP-side time() captured by the test by a second or two.
        $this->assertEqualsWithDelta($expected, (int)$actual, 2, $message);
    }
    ...
    public function testObjectKeepUpdateDateUnchanged()
    {
        ...
        $tsave = time();
        $t1->save();
        // With native ON UPDATE CURRENT_TIMESTAMP the new value is written by the database
        // server, so the in-memory object is stale until reloaded.
        Table2TableMap::clearInstancePool();
        $t1 = Table2Query::create()->findPk($t1->getId());
